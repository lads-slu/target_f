...
for(i in ff){
  ...
  sel1<-a<=5; sel1[is.na(sel1)]<-FALSE;plot(b[s,],main =i, xlab=n); plot(s[!sel1,],add=T);s[,paste0('o_',n)]<-!sel1 
  #n<-'stop'; a<-as.data.frame(s)[,n]; qq<-quantile(a[], probs=c(0.05, 0.95), names=F, na.rm=T)
  #sel2<-a<=40; sel2[is.na(sel2)]<-FALSE;plot(b[s,],main =i, xlab=n); plot(s[!sel2,],add=T);s[,paste0('o_',n)]<-!sel2 
  plot(x=1, y=1, main ='no time slot')
  
  n<-'gap'; a<-as.data.frame(s)[,n];qq<-quantile(a[], probs=c(0.025, 0.975), names=F, na.rm=T)
  sel3<-a<=qq[2]; sel3[is.na(sel3)]<-FALSE; plot(b[s,],main =i, xlab=n); plot(s[!sel3,],add=T);s[,paste0('o_',n)]<-!sel3 
  
  n<-'density'; a<-as.data.frame(s)[,n]
  if(sum(is.na(values(s[,n])))<0.5*nrow(s)){
    qq<-quantile(a[sel], probs=c(0.025, 0.975), names=F, na.rm=T)
    sel4<-a<qq[2]
    plot(b[s,],main =i, xlab=n); plot(s[!sel4,],add=T)
  }else{
    sel4<-rep(TRUE,length(a));sel4[is.na(sel4)]<-FALSE
  ...
  if(sum(is.na(values(s[,n])))<0.5*nrow(s)){
    qq<-quantile(a[sel], probs=c(0.025, 0.975), names=F, na.rm=T)
    selk<-qq[2];selk[is.na(selk)]<-FALSE
