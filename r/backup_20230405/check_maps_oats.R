...
blockname<-"block_oats19_21_v2.shp" 
yieldname<-"YIELD"
...
for(i in ff){
  ...
  #rename columns
  nnold<-c(yieldname, moisturename, timename, distancename, headingname, widthname)
  nnnew<-c('yield', 'moisture', 'time', 'distance', 'heading', 'width')
  for (j in 1:length(nnold)){
    nn<-nnold[j]
    if(!is.na(nnold[j])){
      names(s)[names(s)==nnold[j]]<-nnnew[j]
      }
  }
  
  #compute heading
  xy0<-as.data.frame(crds(s)) #precious point 
  ...
  sel4<-a<=30; plot(b[s,],main =i, xlab=n); if(sum(!sel4)>0) plot(s[!sel4,],add=T);s[,paste0('o_',n)]<-!sel4
  n<-"width"; a<-as.data.frame(s)[,n]; sel5<-a>=0.95*max(a); plot(b[s,],main =i, xlab=n); if(sum(!sel5)>0) plot(s[!sel5,],add=T); s[,paste0('o_',n)]<-!sel4 
  ...
  n<-'yield'; a<-as.data.frame(s)[,n];qq<-quantile(a[sel], probs=c(0.025, 0.975), names=F, na.rm=T)
