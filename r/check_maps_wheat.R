...
for(i in ff){
  ...
  n<-'turn'; a<-as.data.frame(s)[,n]; sel1<-a<=3; plot(b[s,],main =i, xlab=n); plot(s[!sel1,],add=T);s[,paste0('o_',n)]<-!sel1 
  n<-'DISTANCE'; a<-as.data.frame(s)[,n]; sel2<-a>=0.5; plot(b[s,],main =i, xlab=n); plot(s[!sel2,],add=T);s[,paste0('o_',n)]<-!sel2 
  n<-'density'; a<-as.data.frame(s)[,n]; sel3<-a<=15; plot(b[s,],main =i, xlab=n); if(sum(!sel3)>0) plot(s[!sel3,],add=T);s[,paste0('o_',n)]<-!sel3 
  n<-'SWATHWIDTH'; a<-as.data.frame(s)[,n]; sel4<-a>=8.5; #plot(b[s,],main =i, xlab=n); if(sum(!sel4)>0) plot(s[!sel4,],add=T)
  s[,paste0('o_',n)]<-!sel4 
