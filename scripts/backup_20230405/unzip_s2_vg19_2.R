...
#define objects
s2dir<-"s2\\vg19"
ydir<-"data\\ymaps19\\oats19\\3_epsg3006_cleaned"
testdir<-"data\\ymaps19\\oats19\\3_epsg3006_cleaned\\testdata"
ptsname<-file.path(ydir, 'sall_cleaned.shp')
blockname<-file.path(ydir, 'block_oats19.shp')
datetouse<-'20190714' #'20210628' '2021070'
...
ff<-list.files(s2dir,pattern = ".jp2", recursive = TRUE) #select jp2-files
sel<-grepl(datetouse, ff); ff<-ff[sel] #select date
sel<-grepl('B07_20m', ff); fname<-file.path(s2dir, ff[sel]) #select b7-file
b7<-rast(fname) #read b7-file
sel<-grepl('B05_20m', ff); fname<-file.path(s2dir, ff[sel]) #select b5-file
...
for (i in 1:nrow(b)){
  fname<-file.path(testdir, paste0('testfield', i, '.shp'))
  writeVector(x=b[i,], filename=fname, overwrite=TRUE)
}
fname<-file.path(testdir, paste0('testndre75', '.tif'))
writeRaster(x=r, filename=fname, datatype="FLT4S", overwrite=TRUE)
...
#export data for testing
fname<-file.path(testdir, paste0('testcombine', '.tif'))
writeRaster(x=rr$combine, filename=fname, datatype="FLT4S", overwrite=TRUE)
